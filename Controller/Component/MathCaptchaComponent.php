...
class MathCaptchaComponent extends Component {
    ...
    public function makeCaptcha($autoRegister = true) {
        ...
        $choices = array(
            __("What is the sum of") . " .:addition-problem:.?",
            __("What does") . " .:word-problem:." . __(" result in?"),
            __("What is") . " .:problem:.?",
            __("What's the result of") . " .:word-problem:.?",
            __("What do you get when you") . " .:word2-problem:.?"
        );
        
        $question = $choices[mt_rand(0, count($choices) - 1)];
        $this->convertPlaceholders($question);

        if ($autoRegister && !$this->settings['tabsafe'])
            $this->registerAnswer();
    ...
    private function convertPlaceholders($string, $maxrepeat = -1, $repitition = 0) {
        ...
        ".:operatorword:." => array(
             __("add") . " .:number:." . __(" to") => "+",
             __("multiply") . " .:number:." . __(" by") => "*"),
        ".:operatorword-ing:." => array(
             __("adding") . " .:number:." .  __(" to") => "+",
             __("multiplying") . " .:number:." .  __(" by") => "*")
    );
        ...
        if ($loose && !$validated) {
            if (is_array($this->alternatives[$result])) {
                foreach ($this->alternatives[$result] as $alternative) {
                    if (strcasecmp($data, $alternative) == 0)
                        $validated = true;
                }
            } else {
                if (strcasecmp($data, $this->alternatives[$result]) == 0)
                    $validated = true;
            }
        }

        if ($validated && $autoUnset)
            $this->unsetAnswer();
        return $validated;
    }
    ...
    private function validateTabsafe($data, $loose, $autoUnset) {
        $alternatives = array(
        0 => array(__("zero"), "O", __("null"), __("nil"), __("nada"), __("zip"), __("zilch"), __("nothing"), __("rien"), __("naught")),
        1 => __("one"),
        2 => __("two"),
        3 => __("three"),
        4 => __("four"),
        ...
        10 => __("ten"),
        12 => __("twelve"),
        15 => array(__("fifteen"), __("fivteen")),
        16 => __("sixteen"),
        20 => __("twenty"),
        25 => array(__("twentyfive"), __("twenty-five"))
    );
        $result = $data[1];
        $data = $data[0];
                ...
                if ($result == md5($key)) {
                    if (is_array($value)) {
                        foreach ($value as $alternative) {
                            if (strcasecmp(md5($data), md5($alternative)) == 0)
                                $validated = true;
                        }
                    } else {
                        if (strcasecmp(md5($data), md5($value)) == 0)
                            $validated = true;
                    }
                }
            }
