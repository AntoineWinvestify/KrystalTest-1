...
class Contactform extends AppModel {
     ...
     * 							
     */
    public function createContactMessage($userName, $email, $subject, $text) {
        $data = array(
            'name' => $userName,
            'email' => $email,
            'subject' => $subject,
    ...
    function afterSave($created, $options = array()) {
        ...
        if ($created) {
            $event = new CakeEvent("sendContactMessage", $this, array('name' => $this->data['name'], 'email' => $this->data['email'], 'subject' => $this->data['subject'], 'text' => $this->data['text']));
            $this->getEventManager()->dispatch($event);
        }
        return true;
    }
